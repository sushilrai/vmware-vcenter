...
Puppet::Type.type(:vc_vm).provide(:vc_vm, :parent => Puppet::Provider::Vcenter) do
  ...
  def get_cluster_datastore
    ...
    data.select { |d| d['summary.accessible'] }.sort_by { |d| d['info.url'] }.each do |d|
      ...
      datastore_info[d['name']] = { 'size' => size, 'free' => free, 'used' => used}
    end
    ...
    if !requested_datastore.empty?
      ...
      datastore_info.keys.each do |datastore|
        Puppet.debug("Datastore : #{datastore}")
